...
def play(word):
        ...
        if n.isalpha():
            if len(n) == 1:
               if n in guessed_letters:
                  print('Эта буква уже писалась! Попробуйте что-то новое.')
               if n in word and n not in guessed_letters:   
                  for j in range(len(word)):
                     if n == word[j]:
                        word_completion = word_completion[:j] + word[j] + word_completion[j + 1:]
                  guessed_letters.append(n)
               if n not in word and n not in guessed_letters:
                  tries -= 1
                  guessed_letters.append(n)
            ...
            if len(n) > 1:
               if n == word:
                  ...
                  print('Поздравляем, вы угадали слово! Вы победили!')
                  break
               if n != word and n not in guessed_words:
                  tries -= 1
                  guessed_words.append(n)
               else:
                  print('Это слово уже называлось! Попробуйте что-то новое.')
        else:
    ...
    if tries == 0:
       print(word)
       print('К сожалению, вы прогирали!')
    global what 
    what = input('Желаете сыграть еще раз? +/- (другой символ будет расцениваться как -)')
